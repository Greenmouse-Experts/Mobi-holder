...
export default function TicketEvent({ back, data }) {
  ...
  // Auto set price to 0 when plan is Free
  useEffect(() => {
    if (selectedPlan === "Free") {
      ticketsArray.forEach((_, index) => {
        setValue(`price${index}`, "0");
      });
    }
  }, [selectedPlan, ticketsArray, setValue]);
  ...
  const checkEvent = (data) => {
    // setIsLoading(true);
    // return console.log(ticketsArray);
    openModal({
      size: "sm",
      content: (
        <ConfirmModal
      ...
      <form onSubmit={handleSubmit(checkEvent)}>
        <div className="mb-1 flex flex-col gap-10 mt-5">
          ...
          {isRecurring && (
            <>
              <div className="flex flex-col w-full gap-6">
                <p className="-mb-3 text-mobiFormGray">Event Frequency</p>
                <Input
                  name="frequency"
              ...
              {recurrenceType === "Count" && (
                <div className="flex flex-col w-full gap-6">
                  ...
                  <Input
                    ...
                    rules={{ required: "Number of Recurrencies is required" }}
                    errors={errors}
                    register={register}
              ...
              {recurrenceType === "Date" && (
                <div className="flex flex-col w-full gap-6">
                  <p className="-mb-3 text-mobiFormGray">Recurrence End Date</p>
                  <Input
                    type="datetime-local"
                    name="recurrenceEndDate"
                    rules={{ required: "Recurrency end date is required" }}
                    errors={errors}
                    register={register}
          ...
          {ticketsArray.map((tickets, index) => (
            <div className="w-full flex flex-col rounded-lg gap-8 -mt-1 p-3 border-2 border-dashed">
              <div className="flex flex-col w-full gap-6" key={index}>
                <p className="-mb-3 text-mobiFormGray">Ticket Name</p>
                <Input
                  type="text"
              ...
              <div className="flex flex-col w-full gap-6">
                ...
                <Input
                  type="text"
                  name={`ticketsAvailable${index}`}
                  rules={{
                    required: "Number of Available Tickets is required",
                  }}
                  errors={errors}
                  register={register}
              ...
              <div className="flex flex-col w-full gap-6">
                ...
                <Input
                  type="text"
                  name={`plusAllowed${index}`}
                  rules={{
                    required:
                      "Additional Number of Available Tickets is required",
                  }}
                  errors={errors}
                  register={register}
              ...
              {selectedPlan === "Paid" && (
                <div className="flex flex-col w-full gap-6">
                  <p className="-mb-3 text-mobiFormGray">Price of Ticket</p>
                  <Input
                    type="text"
                    name={`price${index}`}
                    rules={{ required: "Price of Ticket is required" }}
                    errors={errors}
                    register={register}
              ...
              {index > 0 && (
                <Button
                  onClick={() => removeTicketTypes(index)}
                  className="md:w-1/3 w-full -mt-2 p-2 bg-red-500 rounded-lg"
                >
                  Remove Ticket Type
                </Button>
              )}
          ...
          <div className="flex gap-8 mb-8">
            <Button
              onClick={async () => await addTicketTypes()}
              className="md:w-1/3 w-full p-4 rounded-lg"
            >
          ...
          <div className="flex gap-8">
            <Button
              onClick={() => handleBack()}
              className="md:w-1/4 w-full p-3 rounded-full"
            >
              Previous
            </Button>
