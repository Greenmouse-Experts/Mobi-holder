...
export default function OrgVerifyEvent() {
    ...
    const getEventTicketsRequest = () => {
        mutate({
            url: `/api/verifications/verification/request/by/event?eventId=${id}`,
            method: "GET",
            headers: true,
            hideToast: true,
            onSuccess: (response) => {
    ...
    const scanEventTicket = (data) => {
        setIsLoading(true)
        const payload = {
            ...data,
            verificationMethod: 'ticket_id'
        };

        mutate({
            url: `/api/verifications/scan/event/request`,
            method: "POST",
            headers: true,
            data: payload,
            onSuccess: (response) => {
        ...
        <>
            <div className="w-full flex h-full animate__animated animate__fadeIn">
                <div className="w-full flex flex-col gap-5 h-full">
                    <Header mobile organisation data={user} title={'Scan Event'} />
                    <div className="w-full flex justify-between items-center gap-8 md:my-5 my-2 px-3">
                        <div className="w-full flex flex-col gap-2">
                            <p className="lg:text-2xl md:text-xl text-lg font-semibold md:hidden">Scan Event</p>
                            <p className="text-base">Scan Users for: <span className="text-mobiBlue">{eventDetails.name}</span></p>
                        </div>
                    </div>
                    ...
                    <div className="w-full flex flex-grow md:flex-row flex-col md:px-0 px-3 justify-between items-start gap-8">
                        ...
                        <div className="shadow-xl md:py-5 md:px-8 px-2 py-2 md:w-[70%] w-full border border-mobiBorderFray card-body flex rounded-xl flex-col gap-10">
                            <form onSubmit={handleSubmit(scanEventTicket)}>
                                <div className="mb-1 flex flex-col gap-10 mt-5">
                                    <div className="flex flex-col w-full gap-6">
                                        ...
                                        <Input type="text" name="eventRequestId" register={register} errors={errors} rules={{ required: 'Ticket ID is required' }} placeholder="Enter User Ticket ID" />
                                    </div>
                                    <div className="flex">
