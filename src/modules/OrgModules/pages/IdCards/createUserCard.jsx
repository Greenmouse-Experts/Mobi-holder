...
export default function CreateUserCard() {
  ...
  const templates_query = useQuery({
    queryKey: ["templates", "card"],
    queryFn: async () => {
      let resp = await newApi.get("/api/idcards/templates");
      return resp.data;
    },
  });
  const card_templates = templates_query.data?.data?.map((item) => ({
    label: item.name,
    value: item.id,
  }));
  useEffect(() => {
    console.log("card_temps", card_templates);
  }, [card_templates]);
  ...
  const eventDetails = [
    {
      icon: (
        ...
        >
          <path
            d="M1.8 7.15V8.45H12.2V7.15H1.8ZM4.4 13V9.75H0.5V0H13.5V9.75H9.6V13L7 11.7L4.4 13ZM1.8 5.2H12.2V1.3H1.8V5.2Z"
            fill="#A1A1A1"
          />
        </svg>
      ),
    ...
    },
    {
      ...
      name: (
        <span>
          Status :{" "}
          <span
            className={`${individualData.status === "active" ? "text-[rgba(76,217,100,1)]" : "text-red-500"} capitalize`}
          >
            ...
            <div className="w-full flex flex-col gap-2">
              ...
              <p className="text-base">
                ...
                <span className="text-mobiBlue">
                  {individualData.individual.firstName}
                  {individualData.individual.lastName}
                </span>
              </p>
            </div>
          </div>
              ...
              <div className="w-full flex justify-center">
                <div className="w-[200px] h-[200px] flex rounded-full bg-gray-400">
                  {individualData.individual.photo ? (
                    <img
                      src={individualData?.individual.photo}
                  ...
                  <div className="w-full flex flex-col gap-6">
                    <div className="flex flex-col w-full gap-6">
                      ...
                      <Input
                        type="text"
                        name="designation"
                        value={individualData?.designation}
                        register={register}
                        placeholder="Enter role in organisation"
