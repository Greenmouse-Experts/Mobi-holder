        ...
        <>
          <div className="max-w-lg mx-auto bg-white rounded-xl p-6">
            ...
            <p className="text-gray-600 mb-4">
              Before you delete your account, is there anything we can do to
                ...
                >
                  {tabs.map((tab, index) => (
                    <div
                      ...
                      className={`flex items-center text-black py-2 cursor-pointer px-4 h-[40px] rounded-lg transition`}
                    >
                      <span>{tab.name}</span>
                    </div>
                  ))}
            ...
            <div className="w-full flex gap-10 items-start">
              <div className="bg-mobiDarkCloud w-[23%] rounded-md lg:flex md:flex hidden flex-col gap-3 px-2 py-5">
                <nav className="px-1 space-y-4">
                  {tabs.map((tab, index) => (
                    <div
                      key={index}
                      onClick={() => setActiveTab(tab.slug)}
                      className={`flex items-center py-2 cursor-pointer px-4 h-[40px] rounded-lg ${activeTab === tab.slug ? "bg-mobiBlueFade" : "hover:bg-mobiBlueFade text-mobiRomanSilver"} transition`}
                    >
                      <span
                        className={`${activeTab === tab.slug ? "text-mobiPink font-[500]" : ""}`}
                      >
                        {tab.name}
                      </span>
                    </div>
                ...
                <nav className="px-1 space-y-4">
                  <div
                    onClick={() => handleDeleteModal()}
                    className={`flex items-center py-2 cursor-pointer px-4 h-[40px] rounded-lg transition`}
                  >
                    <span className="text-red-500">Delete Account</span>
                  </div>
                </nav>
