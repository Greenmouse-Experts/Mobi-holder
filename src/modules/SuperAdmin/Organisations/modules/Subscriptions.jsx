...
export default function Subscriptions() {
    const RequetsHeaders = ["Plan Name", "Plan Duration", "Amount", "Date Created"];
    const RequetsHeaders1 = ["Transaction ID", "Members Name", "MBIH", "Date", "Plan Name", "Amount", "Status"];
                ...
                <Table width="lg:w-[1000px]" title="Today" transparentBg filter subTitle={<span>Subscriptions Plans</span>} exportData
                    ...
                    {NewTableData.map((data, index) => (
                        <tr key={index} className={`py-5 ${index % 2 === 0 ? 'bg-mobiDarkCloud' : 'bg-mobiTheme'}`}>
                            ...
                            <td className="px-3 py-3 text-mobiTableText whitespace-normal">{data.duration}</td>
                ...
                <Table width="lg:w-[1000px]" title="Today" transparentBg filter subTitle={<span>Subscriptions Log</span>} exportData
                    ...
                    {NewTableData2.map((data, index) => (
                        <tr key={index} className={`py-5 ${index % 2 === 0 ? 'bg-mobiDarkCloud' : 'bg-mobiTheme'}`}>
                            ...
                            <td className="px-3 py-3 text-mobiTableText whitespace-normal">{data.name}</td>
                            <td className="px-3 py-3 text-mobiTableText whitespace-normal">{data.mbih}</td>
                            <td className="px-3 py-3 text-mobiTableText whitespace-normal">{data.number}</td>
