      ...
      <div className="w-full flex flex-col gap-5 h-full">
        ...
        <div className="w-full flex flex-col gap-5 border border-mobiBorderFray card-body p-5 rounded-xl my-2">
          ...
          <div className="w-full flex lg:flex-row md:flex-row flex-col gap-5">
            <Table
              filter
              subTitle={<span>Subscription Log</span>}
              exportData
            ...
            >
              {subscribers.map((data, index) => (
                ...
                >
                  ...
                  <td className="px-3 py-3 text-mobiTableText">
                    {data.plan.duration} month(s)
                  </td>
                  <td className="px-3 py-3 text-mobiTableText">
                    {dateFormat(data.subscribedAt, "dd MMM, yyy")}
                  </td>
                  <td className="px-3 py-3 text-mobiTableText">
                    {dateFormat(data.expiresAt, "dd MMM, yyy")}
                  </td>
                  <td className="px-3 py-3 text-mobiTableText">
                    {data.plan.currency} {data.plan.amount}
                  </td>
                  <td className="px-3 py-3 text-mobiTableText">
                    <Badge
                      status={data.status}
                      color={data.status === "active" ? "#4CAF50" : "inactive"}
                      textColor={
                        data.status === "active" ? "#FFFFFF" : "#000000"
                      }
                    />
                  </td>
                </tr>
...
export default OrganisationLog;
