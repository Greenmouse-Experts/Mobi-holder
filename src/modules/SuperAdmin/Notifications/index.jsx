...
export default function Notification() {
  ...
  const markRead = (id) => {
    mutate({
      url: `/api/admins/notification/read?id=${id}`,
      method: "PATCH",
      headers: true,
      onSuccess: (response) => {
        console.log(response);
  ...
  const handleMarkAllReadClick = () => {
    mutate({
      url: `/api/admins/notifications/mark-all-read`,
      method: "PATCH",
      headers: true,
      onSuccess: (response) => {
        const updatedNotifications = allNotifications.map((notification) => {
              ...
              <div className="bg-mobiDarkCloud md:my-3 lg:w-3/4 md:w-3/4 w-full rounded-lg">
                <div className="flex justify-between items-center px-6 py-5">
                  ...
                  <div className="flex space-x-2">
                    <p className="text-sm underline cursor-pointer font-[500] text-mobiSkyBlue">
                      Mark All as Read
                    </p>
                  </div>
                </div>
                ...
                <ul className="space-y-2 flex flex-col md:p-6 p-3 gap-3">
                  {allNotifications.map((notification, index) => (
                    <li
                      key={index}
                      className={`flex items-center relative gap-2 py-3 px-3 rounded-md transition-all duration-200 ${
                        notification.read ? "border border-mobiNotification" : "bg-mobiUnread"
                    ...
                    >
                      ...
                      <div className="flex-1">
                        <p
                          ...
                          } line-clamp-1 group-hover:line-clamp-none transition-all duration-200 cursor-pointer`}
                          title={notification.message}
                        >
                          {notification.message}
                        </p>
                        <p className="text-mobiSkyBlue text-xs group-hover:text-sm transition-all duration-200">
                          {notification.time}
                        </p>
                      </div>
                      ...
                      {!notification.read ? (
                        <span className="w-[13px] h-[13px] rounded-full bg-mobiPink absolute top-[3px] right-1" />
                      ) : null}
                    </li>
                  ))}
                </ul>
