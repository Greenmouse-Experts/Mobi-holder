...
export default function EventInvites() {
    ...
    const handleDeclineModal = (invitationToken) => {
        openModal({
            size: "sm",
            content: <AlertModal closeModal={closeModal} title={'Decline Invitation'}
                redirect={getEventInvites} api={`/api/events/invitation/respond`}
                method={'PUT'} body={{ token: invitationToken, status: 'Declined' }}
    ...
    const handleAcceptModal = (data) => {
        ...
        openModal({
            size: "sm",
            content: data.isFree ?
                <AlertModal closeModal={closeModal} title={'Accept Invitation'}
                    redirect={getEventInvites} api={`/api/events/invitation/respond`}
                    method={'PUT'} body={{ token: data.invitationToken, status: 'Accepted' }}
                ...
                <div className="w-full flex h-auto flex-col px-3 py-6 gap-3 -mt-3">
                    <div className="text-center w-full flex flex-col gap-3">
                        <p className="font-semibold text-center text-lg">Accept Invitation</p>
                        <p className="text-base">Do you want to accept this invitation?</p>
                        <p className="text-sm text-gray-800">You will be charged <span className="font-semibold text-mobiPink">₦{data?.eventtickets?.price}</span> for this event.</p>
                    </div>
    ...
    const handlePayment = (event) => {
        const paystackConfig = {
            ...
            amount: Number(event?.eventtickets.price) > 0 ? Number(event?.eventtickets.price) * 100 : 0,
    ...
    const onSuccess = (reference) => {
        mutate({
            ...
            method: "PUT",
            headers: true,
            data: { token: selectedEvent.invitationToken, status: 'Accepted', refID: reference.reference },
            onSuccess: (response) => {
                getEventInvites();
                closeModal();
            },
    ...
    const TableHeaders = ["Event Name", "Ticket Type", "Invited On", "Status", "Action"];
                                ...
                                eventInvites
                                    .map((data, index) => (
                                        <tr key={index} className={`py-5 ${index % 2 === 0 ? 'bg-mobiDarkCloud' : 'bg-mobiTheme'}`}>
                                            ...
                                            <td className="px-5 py-3 text-mobiTableText">{data.event.ticketType}</td>
                                            <td className="px-5 py-3 text-mobiTableText">{dateFormat(data.invitedAt, 'dd-MM-yyy')}</td>
                                            <td className="py-3 text-mobiTableText">
                                                {data.status === 'Pending' ?
                                                    <span className="flex gap-2">
                                                        <span onClick={() => handleDeclineModal(data.invitationToken)} className="flex cursor-pointer py-2 px-3 rounded-full border border-[rgba(247,77,27,1)]">
                                                            <p className="text-[rgba(247,77,27,1)] text-xs font-[500]">Decline</p>
                                                        </span>
                                                        <span onClick={() => handleAcceptModal(data)} className="flex cursor-pointer py-2 px-3 rounded-full bg-mobiPink">
                                                            <p className="text-white text-xs font-[500]">Accept</p>
                                                        </span>
                                                    </span>
                                                    :
                                                    <Badge status={data.status} color={data.status === 'Accepted' ? 'bg-green-500 text-white' : data.status === 'Pending' ? 'bg-yellow-500 text-white' : 'bg-red-500 text-white'} />
                                                }
                                            </td>
