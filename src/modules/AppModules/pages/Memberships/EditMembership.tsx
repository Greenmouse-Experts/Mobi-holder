...
import { IndDashContainer } from "../../Dashboard/layouts/IndDashContainer";
...
export default function EditMembership() {
  ...
  const query = useQuery<EDIT_RESPONSE>({
    ...
    queryFn: async () => {
      let resp = await newApi.get(
        `/api/memberships-subscriptions/individual/membership?search=${id}`,
      );
      return resp.data;
    },
  });
  ...
  if (query.isError)
    return (
      <>
        <ErrorComponent refetch={query.refetch} />
      </>
    );

  return (
    <>
      <IndDashContainer title="Edit MemberrShip">
