...
export default function ContactUs() {
  const [supportContacts, setSupportContacts] = useState({
    phones: ["0700 000 0000"], // Default fallback array
    emails: ["infomobiholder@gmail.com"], // Default fallback array
    address: "Lekki , Lagos", // Default fallback
    address2: "London, UK", // Default fallback
  });
  const { mutate } = useApiMutation();
    ...
    mutate({
      ...
      onSuccess: (response) => {
        ...
        if (response.data && response.data.data && Array.isArray(response.data.data)) {
          ...
          setSupportContacts({
            phones: phones.length > 0 ? phones : ["0700 000 0000"],
            emails: emails.length > 0 ? emails : ["infomobiholder@gmail.com"],
            address: "Lekki , Lagos", // Keep default as API doesn't provide address
            address2: "London, UK", // Keep default as API doesn't provide address
          });
        }
      },
      onError: (error) => {
      ...
      <div className="flex flex-col w-full h-full animate__animated animate__fadeIn">
        <div className="w-full h-full relative">
          <div
            ...
            style={{
              backgroundImage: `url(https://res.cloudinary.com/do2kojulq/image/upload/v1736029463/mobiHolder/mobiHolder_home/spring-ball-roller_d3dhsf.gif)`,
            }}
          ></div>
          <div
            className="absolute w-full md:top-[0px] top-[20px] h-full"
          ...
          <div className="w-full lg:mt-10 md:mt-10 mt-[40px] px-8 lg:py-10 relative lg:px-44 xl:px-72 md:px-20 flex flex-col gap-10 z-50">
            <p className="md:text-5xl text-3xl text-white font-bold mb-5 md:leading-[90px]">
              Contact Us
            </p>
          </div>
        </div>
        ...
        <div className="w-full h-full relative">
          <div
            className="w-full h-full flex md:flex-row justify-center flex-col gap-8 md:py-14 md:px-32 px-2 relative"
            style={{ backgroundColor: "rgba(249, 247, 243, 1)" }}
          >
                    ...
                    <div className="flex flex-col gap-1">
                      {supportContacts.emails.map((email, index) => (
                        <a 
                          key={index}
                          href={`mailto:${email}`} 
                          className="text-base text-white hover:text-blue-300 transition-colors break-all"
                        >
                          {email}
                        </a>
                      ))}
